...
여러 값들의 집합을 펼쳐서 개발적인 값의 목록으로 만드는 문법입니다.

## 구조 분해 할당

### 구조 분해 할당이 뭔가요?

배열 또는 객체를 비구조화하여 1개 이상의 변수에 개별적으로 할당하는 것을 말합니다.
필요한 값만 추출하여 변수에 할당할 때 사용합니다.

## 브라우저 렌더링 과정

### 브라우저의 렌더링 과정에 대해 설명해보세요

브라우저는 먼저 DNS 서버에 요청을 보내고, 그에 대한 IP 주소를 수신합니다.
브라우저는 받은 IP 주소를 통해 웹 서버에게 GET 요청을 전송합니다.
요청을 받은 웹 서버는 이에 해당하는 HTML, CSS, JavaScript 파일과 웹 페이지를 구성하는 여러 리소스를 응답합니다.
...
script 태그에서 DOM 요소를 조작하는 경우, body 태그보다 위에 위치하면 해당 DOM 요소가 아직 로드되지 않아 에러가 발생할 수 있습니다.
이 때문에 일반적으로 script 태그는 body 태그 아래에 배치합니다.

하지만 script 태그에 defer 속성을 사용하면 HTML 문서의 파싱이 완료된 후 스크립트가 실행되므로, script 태그의 위치와 관계없이 안전하게 DOM 요소를 조작할 수 있습니다.
